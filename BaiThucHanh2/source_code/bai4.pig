...
parsed = FOREACH raw_line GENERATE
    ...
    REGEX_EXTRACT(line, '^(\\d+);(.*);([^;]+);([^;]+);([^;]+)$', 3) AS category,
    REGEX_EXTRACT(line, '^(\\d+);(.*);([^;]+);([^;]+);([^;]+)$', 4) AS aspect,
...
STORE pos_top5_by_category INTO 'output/output_bai4_top5_positive_words_by_category' USING PigStorage(';');
STORE neg_top5_by_category INTO 'output/output_bai4_top5_negative_words_by_category' USING PigStorage(';');